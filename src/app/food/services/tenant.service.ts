...
import { TenantSubscription } from '../interfaces/tenant-subscription';
...
export class TenantService {
 ...
 // private authService = inject(AuthService)
  ...
  private tenantAdmin = signal<Tenant | any>({});

  get currentTenant() {
    return this.tenant;
  }

  get currentTenantAdmin() {
    return this.tenantAdmin;
  }

  // getTenats(): Observable<TenantSubscription[]> {
  //   const headers = new HttpHeaders().set('Authorization', `Bearer ${this.token}`);
  //   return this.httpClient.get<TenantSubscription[]>(`${this.baseUrl}/tenants` , {headers:headers});
  // }
  ...
  getTenantByName(businessName:string | any):Observable<Tenant | undefined>{
    return this.httpClient
